...
cfg = json.load(open(os.path.join(root_path, Path("config/pw-hostconfig.json"))))
...
try:
    ...
    if not mosquitto == None:
        # info("Setting up Mosquitto ...", mosquitto)
        mqtt = True
except:
    mqtt = False

if mqtt:
    info("MQTT initializied")
...
class PWControl(object):
    ...
    def __init__(self):
        ...
        self.staticconfig_fn = str(root_path.joinpath('config/pw-conf.json'))
        self.control_fn = str(root_path.joinpath('config/pw-control.json'))
    ...
    def ten_seconds(self):
        ...
        for mac, f in self.actfiles.items():
            ...
            if epochf:
                ...
                t = datetime.datetime.time(datetime.datetime.utcnow()-timedelta(seconds=time.timezone))
                ts = 3600*t.hour+60*t.minute+t.second
            try:
                _, usage, _, _ = c.get_power_usage()
                #info("%10d, %8.2f" % (ts, usage,))
                f.write("%5d, %8.2f\n" % (ts, usage,))
    ...
    def log_recording(self, control, mac):
        ...
        if control['savelog'].lower() == 'yes':
            ...
            if last > first + 99:
                last = first + 99
            log = []
            last_dt = datetime.datetime.now().today() - timedelta(days = 1)
            ...
            for dt, watt, watt_hour in log:
                if not dt is None:                
                    ...
                    #info("%s, %s, %s" % (ts_str, watt, watt_hour))
    ...
    def reset_all(self):
        ...
        warn("Untested function, for example purposes")
        warn("Aborting. Remove next line to continue")
        # krak
        #
        #TODO: Exception handling
        for c in self.circles:
            if c.name != 'circle+':
                debug('resetting '+c.name)
                c.reset()
        for c in self.circles:
            if c.name == 'circle+':
                debug('resetting '+c.name)
                c.reset()
        debug('resetting stick')
        self.device.reset()
        debug('sleeping 60 seconds to allow devices to be reset themselves')
        time.sleep(60)

    def init_network(self):
        #NOTE: Untested function, for example purposes
        debug("Untested function, for example purposes")
        debug("Aborting. Remove next line to continue")
        # krak
        #TODO: Exception handling        
        #
        ...
        success = False
        for i in range(0,10):
            info("Trying to connect to circleplus ...")
            ...
            if success:
                ...
                try:
                    ...
                    success = False
            info("sleep 30 seconds for next retry ...")
            time.sleep(30)
    ...
    def connect_node_by_mac(self, newnodemac):
        ...
        info(self.circles[0].read_node_table())
        ...
        try:
            info(self.circles[self.bymac[newnodemac]].get_info())
        except:
            info('new node not detected ...')        
        #
    ...
    def run(self):
        ...
        if circleplus != None:
            try:
                info("Joined node table: %s" % (circleplus.read_node_table(),))
            except:
