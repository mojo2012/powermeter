...
from plugwise.exceptions.ChecksumError import ChecksumError
...
class CompositeType(BaseType):
    ...
    def unserialize(self, val):
        for p in self.contents:
            ...
            p.unserialize(myval)
            logging.debug("PARS      "+ logf(myval) + " EVAL "+str(p.value))
            val = val[len(myval):]
        return val
...
class StringVal(BaseType):
    ...
    def unserialize(self, val):
        try:
            ...
            logging.debug(
                'value error while attempting to construct StringVal object. val = %s'
                % val)
            self.value = 0
...
class UnixTimestamp(Int):
    ...
    def unserialize(self, val):
        ...
        val = self.value
        self.value = datetime.datetime.fromtimestamp(val)
...
class DateTime(CompositeType):
    ...
    def unserialize(self, val):
        ...
        try:
            ...
            logging.debug(
                'value error while attempting to construct datetime object')
            self.value = None
...
class PlugwiseResponse(PlugwiseMessage):
    ID = b'FFFF'
    mac: Union[None, bytes]

    command_counter: Union[str, None]
    ...
    def unserialize(self, response):
        ...
        if len(protocol_error) > 0:
            raise ChecksumError(protocol_error)
...
class PlugwiseLogIntervalRequest(PlugwiseRequest):
    ...
    def __init__(self, mac, usage: int, production: int):
        PlugwiseRequest.__init__(self, mac)
...
class PlugwiseQueryCirclePlusRequest(PlugwiseRequest):
    ...
    def __init__(self):
        ...
        PlugwiseRequest.__init__(self, ''.encode())
