...
class Stick(SerialComChannel):
    ...
    def __init__(self, port="", mac="", timeout=DEFAULT_TIMEOUT):
        self._devtype = 0  # Stick
        self.pan = None
        self.short_pan = None
        self.macAddress = mac.encode()
    ...
    def send_msg(self, cmd):
        ...
        response: Union[PlugwiseAckResponse, None] = None
        success = False
        ...
        if response is not None:
            return (success, response.command_counter)
        else:
            return None
    ...
    def _recv_response(self, retry_timeout=5):
        ...
        while await_response:
            ...
            if header_start > 0:
                ...
                msgHeader = msg[:header_start]
                msgHeaderDecoded = ''

                try:
                    msgHeaderDecoded = msgHeader.decode('utf-8')
                except:
                    pass

                logcomm("DSTR %4d %s" % (len(msgHeader), repr(msgHeaderDecoded)))
    ...
    def expect_response(self, response_class, src_mac=None, seqnr=None, retry_timeout=5):
        ...
        while 1:
            try:
            ...
            except ChecksumError as reason:
                ...
                try:
                    if resp.function_code == b'000E':
                        ...
                        if ackresp.mac is not None and ackresp.mac.decode('utf-8') not in self.unjoined:
                ...
                try:
                    if resp.function_code == b'0000' and not issubclass(resp.__class__, PlugwiseAckResponse):
                        ...
                        if (len(msg) == 22 and msg[0:1] == b'\x05') or (len(msg) == 23 and msg[0:1] == b'\x83'):
                            ...
                            logcomm("RERR %4d %s - <!> unexpected response error: %s" %
                                    (len(msg), logf(msg), str(reason)))
    ...
    def reset(self):
        type = 0
        req = PlugwiseResetRequest(self.macAddress, self._devtype, 20)
        _, seqnr = self.send_msg(req.serialize())
    ...
    def status(self):
        ...
        debug(str(resp))
        self.macAddress = resp.mac.decode('utf-8')
    ...
    def connect_circleplus(self, mac=''):
        if not mac:
            self.circleplusmac = mac

        req = PlugwiseConnectCirclePlusRequest(mac)
        _, seqnr = self.send_msg(req.serialize())
...
class Circle(object):
    ...
    def __init__(self, mac, comchan, attr={}):
        ...
        self.always_on = attr['always_on']
        self.location = attr['location'].strip()
        self.reverse_pol = attr['reverse_pol']
        self.production = attr['production']
        self.loginterval = attr['loginterval']
        shouldInit = attr['init']

        self._devtype = None

        self.gain_a: float = -1
        self.gain_b: float = -1
        self.off_noise: float = -1
        self.off_tot: float = -1
    ...
    def watt_to_pulses(self, watt, seconds=1):
        ...
        corr_pulses_1s: float = watt * PULSES_PER_KW_SECOND / 1000.0

        raw_pulses_1s = (math.sqrt(self.gain_a**2.0 + 4.0 * self.gain_b * (corr_pulses_1s - self.off_tot)) -
                         self.gain_a - 2.0 * self.gain_b * self.off_noise) / (2.0 * self.gain_b)
    ...
    def get_power_usage(self) -> UsageData:
        ...
        return UsageData(
            ...
            watts_1s=kw_1s,
            watts_8s=kw_8s,
            watts_1h=kw_1h,  # this value seems to not be correct
            watts_production_1h=kw_p_1h)
    ...
    def load_schedule(self, dst=0):
        if self.schedule and self.schedule._pulse is not None:
